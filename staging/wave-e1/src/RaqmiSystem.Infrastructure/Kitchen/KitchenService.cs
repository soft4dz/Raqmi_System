    ...
    IStockCostProvider stockCostProvider) : IKitchenService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (from.HasValue)
        {
            query = query.Where(reading => reading.MeasuredAt >= from.Value);
        }

        if (to.HasValue)
        {
            query = query.Where(reading => reading.MeasuredAt <= to.Value);
        }
        ...
        var labels = await LoadCheckpointLabelsAsync(
            readings.Select(reading => reading.CheckpointCode).Distinct().ToArray(),
            cancellationToken);
