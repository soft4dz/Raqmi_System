...
| RBAC | Une permission de lecture existe et protège l'accès : constante `PermissionCatalog.*` (contrôle 6), câblage `ApplyModuleAccess` (contrôle 7) **et** politique API référençant cette constante dans un endpoint déclaré. | `permission` (constante) ; le reste est **dérivé**, jamais saisi | contrôles 6, 7 + lecture des endpoints |
