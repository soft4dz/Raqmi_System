...
public sealed class RecipeIngredientConfiguration : IEntityTypeConfiguration<RecipeIngredient>
{
    public void Configure(EntityTypeBuilder<RecipeIngredient> builder)
    {
        ...
        // JournalEntryLine, FolioCharge and ApprovalDecision): RecipeIngredient assigns its own
        // Id in its constructor, and the lines rebuilt by RecipeSheet.ReplaceIngredients are
        // discovered through the navigation of an ALREADY TRACKED recipe (UpdateRecipeAsync
        // loads the recipe before replacing its lines) with their key already set. With a
        // value-generated key, change tracking would classify those brand-new lines as existing
        // rows (Modified - an UPDATE affecting 0 rows -> DbUpdateConcurrencyException) instead
        // of insertions, and saving an edited recipe would fail.
