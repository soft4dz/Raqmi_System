...
{
    public void Configure(EntityTypeBuilder<PosOutlet> b) { b.ToTable("outlets", "pos"); PosConfig.Audit(b); b.Property(x=>x.Code).HasColumnName("code").HasMaxLength(40).IsRequired(); b.Property(x=>x.Name).HasColumnName("name").HasMaxLength(160).IsRequired(); b.Property(x=>x.HotelUnitCode).HasColumnName("hotel_unit_code").HasMaxLength(40).IsRequired(); b.Property(x=>x.Kind).HasColumnName("kind").HasMaxLength(60).IsRequired(); b.Property(x=>x.WarehouseCode).HasColumnName("warehouse_code").HasMaxLength(40); b.Property(x=>x.IsActive).HasColumnName("is_active"); b.HasIndex(x=>new{x.HotelUnitCode,x.Code}).IsUnique(); b.HasIndex(x=>x.WarehouseCode).IsUnique(); b.HasOne<HotelUnit>().WithMany().HasPrincipalKey(x=>x.Code).HasForeignKey(x=>x.HotelUnitCode).OnDelete(DeleteBehavior.Restrict); b.HasOne<Domain.Inventory.Warehouse>().WithMany().HasPrincipalKey(x=>x.Code).HasForeignKey(x=>x.WarehouseCode).OnDelete(DeleteBehavior.Restrict); }
}
