...
public sealed class InventoryCountLineConfiguration : IEntityTypeConfiguration<InventoryCountLine>
{
    public void Configure(EntityTypeBuilder<InventoryCountLine> builder)
    {
        ...
        // JournalEntryLine, FolioCharge and ApprovalDecision): InventoryCountLine assigns its own
        // Id in its constructor, and the lines of a count are discovered through the navigation of
        // an ALREADY TRACKED InventoryCount (ReplaceCountLinesAsync loads the count before calling
        // count.ReplaceLines) with their key already set. With a value-generated key, change
        // tracking would classify those discovered lines as existing rows (Modified - an UPDATE
        // affecting 0 rows -> DbUpdateConcurrencyException) instead of new ones, and replacing the
        // lines of a count would fail at save time.
