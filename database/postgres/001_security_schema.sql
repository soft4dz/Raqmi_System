...
CREATE TABLE IF NOT EXISTS organization.hotel_units (
    ...
    updated_at timestamptz NULL,
    updated_by varchar(160) NULL,
    CONSTRAINT ck_hotel_units_display_order_non_negative CHECK (display_order >= 0),
    CONSTRAINT ck_hotel_units_unit_type CHECK (unit_type IN ('Hotel', 'Residence', 'BeachClub', 'Marina', 'Other'))
);
...
CREATE TABLE IF NOT EXISTS exploitation.daily_revenues (
    ...
    CONSTRAINT ck_daily_revenues_amounts_non_negative CHECK (
        accommodation >= 0 AND
        food >= 0 AND
        beverage >= 0 AND
        other_revenue >= 0
    ),
    CONSTRAINT ck_daily_revenues_status CHECK (status IN ('Draft', 'Submitted', 'Validated', 'Rejected'))
);
